      ...
      <div className="mb-6 flex flex-wrap items-center gap-4">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            className="w-full rounded-xl border border-input bg-card py-2.5 pl-10 pr-4 text-body text-foreground shadow-soft placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring/50 transition-shadow duration-200"
          />
        </div>
        <div className="flex gap-2">
          {(["all", "low", "moderate", "high"] as RiskFilter[]).map((level) => (
            <button
              ...
              className={`rounded-full px-4 py-2 text-small font-medium capitalize transition-all duration-200 ${
                riskFilter === level
                  ? "bg-primary text-primary-foreground shadow-soft"
                  : "border border-border/60 bg-card text-foreground hover:bg-muted/60"
              }`}
            >
              {level === "all" ? "All" : level}
            </button>
          ...
          <table className="w-full">
            ...
            <tbody>
              {filtered.map((patient, i) => (
                <tr key={i} className="border-b border-border/40 last:border-0 transition-colors duration-200 hover:bg-muted/20">
                  ...
                  <td className="px-6 py-4">
                     <button
                       ...
                       className="flex items-center gap-1.5 rounded-xl bg-primary px-3 py-1.5 text-small font-medium text-primary-foreground shadow-soft transition-all duration-200 hover:shadow-glow hover:-translate-y-0.5"
                     >
